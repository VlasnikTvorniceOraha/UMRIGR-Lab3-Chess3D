...
namespace ChessMainLoop
{
    public class Pawn : Piece
    {
        public override void CreatePath()
        {
            ...
            // ide u smjeru -1, 0
            Piece pieceInFront = BoardState.Instance.GetField(_row + direction, _column);
            if (pieceInFront == null)
            {
                PathManager.CreatePathInSpotDirection(this, direction, 0);

                //provjeri za drugo polje ispred ako se pijun nije pomaknuo
                if (BoardState.Instance.IsInBorders(_row + direction * 2, _column))
                {
                    Piece pieceInFront2 = BoardState.Instance.GetField(_row + direction * 2, _column);
                    if (!HasMoved && pieceInFront2 == null) 
                    {
            ...
            //provjeri tileove jedan dijagonalno od pijuna
            ...
            //en passant, provjeri jesu li figure lijevo ili desno passantable (CreatePassantSpace to vec radi lmao)
